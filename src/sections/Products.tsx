...
import { Zap, Shield, Award, ArrowRight, ShoppingBag, GitCompare, Heart } from 'lucide-react';
...
const Products = () => {
  ...
  const handleGetQuote = (e: React.MouseEvent, p: typeof products[0]) => {
    e.preventDefault();
    e.stopPropagation();
    addToCart({ id: `product-${p.id}`, name: p.name, category: p.category, image: p.image, href: p.href });
    openCart();
  };
  ...
  const handleCompare = (e: React.MouseEvent, p: typeof products[0]) => {
    ...
    if (hasCompare(cid)) removeCompare(cid);
    else addCompare({ id: cid, name: p.name, category: p.category, image: p.image, href: p.href, specs: p.specs, voltage: p.voltage, certification: p.certification, description: p.description });
  };

  const handleWishlist = (e: React.MouseEvent, p: typeof products[0]) => {
    e.preventDefault();
    e.stopPropagation();
    const wid = `product-${p.id}`;
    if (hasWishlist(wid)) removeWishlist(wid);
    else addWishlist({ id: wid, name: p.name, category: p.category, image: p.image, href: p.href });
  };
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className={`flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8 transition-all duration-900 custom-expo ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="flex items-center gap-6">
            ...
            <Link to="/distribution-panels" data-hover className="inline-flex items-center gap-2 text-crimson font-medium text-sm hover:gap-4 transition-all duration-300">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {products.map((p, i) => {
            ...
            const inWishlist = hasWishlist(wid);
            ...
            return (
              <div key={p.id}
                className={`group relative bg-white dark:bg-gray-800 rounded-3xl overflow-hidden border transition-all duration-800 custom-expo card-lift ${
                  inCompare ? 'border-crimson/40 ring-2 ring-crimson/20' : 'border-gray-100 dark:border-gray-700'
                ...
                {inCompare && (
                  <div className="absolute top-3 left-3 z-10 flex items-center gap-1 bg-crimson text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-lg">
                    <GitCompare className="w-3 h-3" /> Comparing
                  </div>
                )}
                ...
                <button
                  ...
                  className={`absolute top-3 right-3 z-10 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${
                    inWishlist
                      ? 'bg-crimson text-white shadow-lg scale-100'
                      : 'bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm text-gray-400 hover:text-crimson opacity-0 group-hover:opacity-100 scale-90'
                  }`}
                >
                  <Heart className={`w-4 h-4 ${inWishlist ? 'fill-white' : 'fill-none'}`} />
                </button>
