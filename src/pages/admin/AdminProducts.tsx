...
const emptyProduct: Omit<Product, 'id'> = {
  name: '', category: CATEGORIES[0], description: '', price: '', capacity: '', voltage: '',
  features: [], image_url: '', href: '', sort_order: 0,
};
...
const AdminProducts = () => {
  ...
  const [modal, setModal] = useState<{ open: boolean; product: Omit<Product, 'id'> & { id?: string } }>({ open: false, product: { ...emptyProduct } });
  ...
  const openNew = () => {
    setModal({ open: true, product: { ...emptyProduct } });
    setFeaturesInput('');
  };
  ...
  const save = async () => {
    ...
    const payload = { ...modal.product, features } as Omit<Product, 'id'> & { id?: string };
    ...
    setSaving(false);
    setModal({ open: false, product: { ...emptyProduct } });
    setLoading(true);
    load();
  };
  ...
  const updateField = (field: string, value: string | number) => {
    setModal(prev => ({ ...prev, product: { ...prev.product, [field]: value } }));
  };
  ...
  return (
    <div className="space-y-6">
                ...
                <tr key={p.id} className="hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(p)} className="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition-colors"><Pencil className="w-4 h-4" /></button>
                      <button onClick={() => deleteProduct(p.id)} className="p-2 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modal.open && (
        <>
          <div className="fixed inset-0 bg-black/60 z-50" onClick={() => setModal({ open: false, product: { ...emptyProduct } })} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto pointer-events-auto p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-white">{modal.product.id ? 'Edit Product' : 'Add Product'}</h3>
                <button onClick={() => setModal({ open: false, product: { ...emptyProduct } })} className="p-2 text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: 'Voltage', field: 'voltage', type: 'text' },
                  { label: 'Image URL', field: 'image_url', type: 'text' },
                  { label: 'Link href', field: 'href', type: 'text' },
                  { label: 'Sort Order', field: 'sort_order', type: 'number' },
                ].map(inp => (
                  <div key={inp.field}>
                    <label className="block text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1.5">{inp.label}</label>
                    <input
                      type={inp.type}
                      value={(modal.product as Record<string, unknown>)[inp.field] as string || ''}
                      onChange={e => updateField(inp.field, inp.type === 'number' ? Number(e.target.value) : e.target.value)}
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setModal({ open: false, product: { ...emptyProduct } })} className="flex-1 py-2.5 border border-gray-700 text-gray-400 rounded-xl hover:bg-gray-800 transition-colors text-sm font-medium">Cancel</button>
