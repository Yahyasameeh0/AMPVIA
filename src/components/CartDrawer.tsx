...
const CartDrawer = () => {
  ...
  const handleQuote = async (e: React.FormEvent) => {
    ...
    if (ok) {
      ...
      alert(`Could not send. Please email us directly at ${FALLBACK_EMAIL}`);
    }
    setSubmitting(false);
  };
  ...
  const formFields = [
    ...
    { label: 'Company',     key: 'company' as const, type: 'text',  required: false },
  ];

  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 bg-black/50 z-[200] backdrop-blur-sm" onClick={close} aria-hidden="true" />

      {/* Drawer */}
      <div role="dialog" aria-modal="true" aria-label="Quote list" className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white dark:bg-gray-900 z-[201] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800">
          ...
          <button onClick={close} data-hover aria-label="Close quote list" className="w-8 h-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center justify-center">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-full overflow-hidden">
                            <button onClick={() => update(item.id, item.qty - 1)} data-hover aria-label="Decrease quantity"
                            ...
                            <button onClick={() => update(item.id, item.qty + 1)} data-hover aria-label="Increase quantity"
                              className="w-7 h-7 flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                              <Plus className="w-3 h-3 text-gray-500" />
                            </button>
                          </div>
                          <button onClick={() => remove(item.id)} data-hover aria-label="Remove item"
        ...
        {step === 'quote' && (
          <form onSubmit={handleQuote} className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              {formFields.map(f => (
                <div key={f.key}>
                  <label htmlFor={`cart-${f.key}`} className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">{f.label}</label>
                  <input id={`cart-${f.key}`} type={f.type} required={f.required}
                    value={form[f.key]}
              ...
              <div>
                <label htmlFor="cart-notes" className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Additional Notes</label>
                <textarea id="cart-notes" rows={3} value={form.notes} maxLength={2000}
