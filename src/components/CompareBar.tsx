  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: 'rgba(10,10,18,0.96)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}
      >
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 flex items-center gap-2 min-w-0">
            {items.map(item => (
              <div key={item.id} className="flex items-center gap-1.5 bg-white/8 border border-white/10 rounded-xl px-2.5 py-1.5 min-w-0">
                ...
                <button onClick={() => remove(item.id)} aria-label={`Remove ${item.name}`} className="flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/10 transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={clear} className="text-xs text-gray-500 hover:text-gray-300 transition-colors px-2 py-1 hidden sm:block">Clear</button>
            <button onClick={openModal} disabled={items.length < 2}
