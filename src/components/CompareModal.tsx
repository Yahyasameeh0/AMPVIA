      ...
      <div className="relative flex flex-col h-full max-h-screen">
        ...
        >
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="min-w-[600px] p-6">
                  ...
                  <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                    <div className="relative h-36 overflow-hidden">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white opacity-0 group-hover:opacity-100 transition-all"
                      >
                    ...
                    <div className="p-4">
                      ...
                      <h3 className="text-white font-semibold text-sm mt-1 leading-tight" style={{ fontFamily: '"DM Serif Display", serif' }}>
                        {p.name}
                      </h3>
                    </div>
                  </div>
            ...
            <div className="space-y-2 mb-8">
              ...
              {SPEC_ROWS.map((row, ri) => (
                <div
                  key={row.key}
                  className={`grid gap-4 px-4 py-3 rounded-xl ${ri % 2 === 0 ? 'bg-white/3' : ''}`}
                  style={{ gridTemplateColumns: `200px repeat(${items.length}, 1fr)` }}
                >
                  <span className="text-xs text-gray-500 font-medium flex items-center">{row.label}</span>
                  {items.map(p => (
            ...
            <div className="space-y-2 mb-8">
              ...
              {FEATURES.map((feat, fi) => (
                <div
                  key={feat.label}
                  className={`grid gap-4 px-4 py-3 rounded-xl items-center ${fi % 2 === 0 ? 'bg-white/3' : ''}`}
                  style={{ gridTemplateColumns: `200px repeat(${items.length}, 1fr)` }}
                >
                  ...
                  {items.map(p => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center gap-1.5">
                        ...
                        <span className={`text-xs font-medium ${yes ? 'text-green-400' : 'text-gray-600'}`}>
                          {yes ? 'Yes' : 'No'}
                        </span>
                      </div>
                    );
            ...
            {/* CTA row */}
            <div
              className="grid gap-4 pt-4 border-t border-white/8"
              style={{ gridTemplateColumns: `200px repeat(${items.length}, 1fr)` }}
            >
              ...
              {items.map(p => (
                <div key={p.id} className="flex flex-col gap-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-2.5 bg-crimson text-white text-xs font-semibold rounded-xl hover:bg-crimson-dark transition-colors"
                  >
                  ...
                  <Link
                    to={p.href}
                    onClick={closeModal}
                    className="flex items-center justify-center gap-1.5 px-4 py-2 border border-white/10 text-gray-400 hover:text-white text-xs font-medium rounded-xl hover:bg-white/5 transition-colors"
                  >
        ...
        {/* Footer */}
        <div
          className="flex-shrink-0 flex items-center justify-between px-6 py-3 border-t border-white/8"
          style={{ background: 'rgba(10,10,18,0.98)' }}
        >
          <p className="text-xs text-gray-600">
            {items.length < 3 ? `You can add ${3 - items.length} more product${3 - items.length > 1 ? 's' : ''} to compare` : 'Maximum 3 products selected'}
          </p>
          <button onClick={clear} className="text-xs text-gray-500 hover:text-crimson transition-colors font-medium">
            Clear all & close
          </button>
        </div>
      </div>
